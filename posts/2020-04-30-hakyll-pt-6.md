...
This is a function parameter with two attributes, `compiler` and `sources`,
that each have defaults. For the `compiler`, we will use this version to compile
all of the Haskell packages that we interact with. For the `sources`, this file
is generated by `niv` and contains our pinned `niv` and `nixpkgs` information.
...
This [nix overlay](https://nixos.org/nixpkgs/manual/#chap-overlays) specifies
that when using `niv` it should use our pinned `niv`.
...
This configures our pinned `nixpkgs` to have some `overrides` for a specific
...
provide an optional list of `patches` (`git diff` files) that we can build
`hakyll` with if there are any changes to the project that we need to make.
These patch files can be empty when no patches are required, but if you do need
to patch something, here is an example `hakyll.patch` file:
...
The `project` attribute is specifically for our Haskell project in order for us
to be sure our project is compiled with our desired compiler version. We
leverage the `callCabal2nix` tool to handle converting our
`hakyll-nix-example.cabal` file into a `nix` derivation.
...
This gives us a variable to use instead of writing
`pkgs.haskell.packages.${compiler}` over and over again.
...
with tools like `nix-build`. All we do is pull our `project` attribute off of
the overridden `nixpkgs` config.
